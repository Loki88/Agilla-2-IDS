...
implementation {
	...
	#ifndef AGILLA_TKN_AM_ID
	#define AGILLA_TKN_AM_ID 14
	#endif

	#ifdef TKN154
		components ActiveMessageC as AM;

		NIM.AMSend -> AM.AMSend[AGILLA_TKN_AM_ID];
		NIM.Receive -> AM.Receive[AGILLA_TKN_AM_ID];
		NIM.AMPacket -> AM;
		NIM.Packet -> AM;
		
		StdControl = AM;
	#else
		components new AMSenderC(AGILLA_TKN_AM_ID) as AMS, new AMReceiverC(AGILLA_TKN_AM_ID) as AMR;
	...
	Init = MessageBufferM;
	AMSend = NIM.BufferedSendMsg;
	Receive = NIM.BufferedReceiveMsg;
	// NIM.AMSend -> AMS.AMSend;
	// NIM.Receive -> AMR.Receive;
	// NIM.AMPacket -> AMS;
	// NIM.Packet -> AMS;
